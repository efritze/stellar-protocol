...
Certain anchors may provide users with the option to  deposit or withdraw assets in and out of the Stellar network, directly from their wallets. For such assets, the `transaction_protocols` field is an array listing all transfer protocols the anchor supports for that asset. 
